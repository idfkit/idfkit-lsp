...
**Fixed since**: the component is `@idfkit/language`, reached at `@idfkit/idfkit/language`, exactly on the
...
registry's similarity filter refused the unscoped name `idfkit`, npm support confirmed on 2026-09-14
that no exception is possible, and the facade moved to `@idfkit/idfkit`, which has not been
published yet, while the scoped packages ship on schedule.
...
`service.ts` and `inputs.ts` and nowhere else, and `levels.json` records the facade's first
publication as what closes it. When `@idfkit/idfkit` is published, the second entry in each list is deleted and nothing else changes.
